...
fs.readFile(__dirname + '/input.txt', (err, data) => {
    ...
    var minsByGuard      = {};
    var worstGuard       = {id: null, total: null, minute: {key: null, val: null}};
        ...
        while(true) {
            ...
            if (lines[next].indexOf('wakes') !== -1) {
                ...
                for (let ct = 0; ct < mins; ct++) {
                    ...
                    // Check
                    if (!worstGuard.id || minsByGuard[guardId].total > worstGuard.total) {
                        worstGuard.id    = guardId;
                        worstGuard.total = minsByGuard[guardId].total
                    }
                    currMin++
                }
                fall = null;
    ...
    // Take worst guard and assess his mins for best time
    for (var min in minsByGuard[worstGuard.id].mins) {
        if (!worstGuard.minute.key || minsByGuard[worstGuard.id].mins[min] > worstGuard.minute.val) {
            worstGuard.minute.key = min;
            worstGuard.minute.val = minsByGuard[worstGuard.id].mins[min];
        }
    }

    console.log(`Answer: ${worstGuard.id * worstGuard.minute.key}`)
